...
{
    private ObjectManager map;

    private void OnEnable()
    {
        map = (ObjectManager)target;
        EnsureArraySize();
    }
    ...
    public override void OnInspectorGUI()
    {
        ...
        map.objectSize = EditorGUILayout.FloatField("Tile Size", map.objectSize);
        ...
        for (int y = 0; y < map.mapSize.y; y++)
        {
            ...
            {
                BaseObject tile = map.GetObject(x, y);
                BaseObject newTile = DrawTileField(tile, fieldSize);

                if (newTile != tile)
                {
                    Undo.RecordObject(map, "Set Tile");
                    map.SetObject(x, y, newTile);
                }
            }
    ...
    private BaseObject DrawTileField(BaseObject tile, int size)
    {
        ...
        return (BaseObject)EditorGUI.ObjectField(rect, tile, typeof(BaseObject), false);
    }
    ...
    private void EnsureArraySize()
    {
        ...
        var tilesProp = so.FindProperty("objectsFlat");
    ...
    private void ResizeArray(Vector2Int newSize)
    {
        ...
        BaseObject[] newArray = new BaseObject[newLength];
        ...
        for (int y = 0; y < copyY; y++)
        {
            ...
            {
                newArray[y * newSize.x + x] = map.GetObject(x, y);
            }
        }
